...
def create_research_plan(state: ResearchState) -> ResearchState:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You're an expert research planner. Create a concise 5-step plan to answer:
{query}
...
Each step should be a single, clear action that can be completed in one research iteration."""),
        ("human", "{query}")
    ])
    
    chain = prompt | llm
    plan = chain.invoke({"query": query}).content
    
    # Extract steps and clean them up
    steps = []
    for line in plan.split("\n"):
        if line.strip() and line[0].isdigit():
            # Remove the number and any following dots/colons
            step = line.split(".", 1)[1].strip() if "." in line else line.split(":", 1)[1].strip()
            steps.append(step)
    
    return {
        **state,
        "research_plan": steps[:5],  # Ensure we only take 5 steps
        "current_step": 0,
        "status": "researching"
    }
